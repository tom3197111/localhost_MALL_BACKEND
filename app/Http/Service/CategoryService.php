...
class CategoryService
{
              ...
              //echo $v->cate_name;
               ...
               foreach($data as $m=>$n){
                   ...
                   if($n->$field_pid==$v->$field_id){
                       ...
                       //的資料取出
                       $data[$m]["_".$field_name]='|__'.$data[$m][$field_name];
                       //最後放到arr[]內
                       $arr[]=$data[$m];
                   }
               }
    ...
    public function upload($photo)
    {
      ...
      if($photo->getClientOriginalExtension()!=null){
        ...
        //dd($file_name);
        //檔案路徑
        // $file_relative_path ='images\Category\Category'.$file_name;
        //將檔案路徑放入到public資料夾內
        // $file_path=public_path($file_relative_path);
        //裁切圖片並存檔
        //storage_path($file_path)
        Image::make($photo)->fit(295,255)->save(public_path().'/images/Category/Category'.$file_name);
        $re='public/images/Category/Category'.$file_name;
        //dd($re);
        return $re;
      }
    }
    ...
    public function update($cate_id,$input){
        ...
        //例如where('欄位名稱',欄位ID)
        //找到後將接收到的資料更新
        // unlink('/var/www/html/folder/file.txt')
         $field=$this->editfind($cate_id);
         if(isset($field->file_upload)){
            $file = $_SERVER['DOCUMENT_ROOT'] . 'localhost_MALL_BACKEND' . DIRECTORY_SEPARATOR . $field->file_upload;
            unlink($file);
         }
        if(isset($input['file_upload'])){
            $photo=$input['file_upload'];
